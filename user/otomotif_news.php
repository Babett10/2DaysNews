...
$otomotif = query("SELECT posts.id, judul, body, img, view, publish, category.nama_category, author.nama_author
FROM posts
JOIN category ON posts.category_id = category.id
JOIN author ON posts.author_id = author.id
WHERE nama_category = 'otomotif'
...
if (isset($_POST["cari"])) {
    ...
    $otomotif = cariCategory($keyword, $nama_category);
}
...
<head>
    <meta charset="utf-8">
    <title>2DAYNEWS | All Otomotif News</title>
                        ...
                        <form action="" method="POST">
                            <div class="input-group-append">
                                ...
                                <select name="nama_category" class="form-control" hidden>
                                    <option value="Otomotif" <?= (isset($nama_category) && $nama_category == 'Otomotif') ? 'selected' : '' ?>>Otomotif</option>
                                </select>
                ...
                <div class="row mb-3">
                    <div class="col-12">
                        <div class="d-flex align-items-center justify-content-between bg-dark py-2 px-4 mb-3">
                            <h3 class="text-white m-0">Otomotif News</h3>
                        </div>
                    </div>
                    ...
                    <?php else : ?>
                        <?php foreach ($otomotif as $oto) :
                            $text = explode(' ', $oto['body']);
                            $textcut = implode(' ', array_slice($text, 0, 15));
                        ?>
                            <div class="col-lg-4">
                                <div class="position-relative mb-3">
                                    <img class="img-fluid w-100" src="../img/<?= $oto['img']; ?>" style="width: 400px; height: 250px; object-fit: cover;">
                                    <div class="overlay position-relative bg-light">
                                        <div class="mb-2" style="font-size: 14px;">
                                            <a href="#"><?= $oto['nama_category']; ?></a>
                                            <span class="px-1">/</span>
                                            <span><?= date("F d, Y", strtotime($oto['publish'])); ?></span>
                                            <span class="px-1">/</span>
                                            <span><?php echo "Viewed " . $oto['view'] . " times"; ?> </span>
                                        </div>
                                        <a class="h4" href="single.php?id=<?= $oto['id'] ?>"><?= $oto['judul'] ?></a>
