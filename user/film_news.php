...
$film = query("SELECT posts.id, judul, body, img,view, publish, category.nama_category, author.nama_author
FROM posts
JOIN category ON posts.category_id = category.id
JOIN author ON posts.author_id = author.id
WHERE nama_category = 'Film'
...
if (isset($_POST["cari"])) {
    ...
    $film = cariCategory($keyword, $nama_category);
}
...
<head>
    <meta charset="utf-8">
    <title>2DAYNEWS | All Film News</title>
                        ...
                        <form action="" method="POST">
                            <div class="input-group-append">
                                ...
                                <select name="nama_category" class="form-control" hidden>
                                    <option value="Film" <?= (isset($nama_category) && $nama_category == 'Film') ? 'selected' : '' ?>>Film</option>
                                </select>
                ...
                <div class="row mb-3">
                    <div class="col-12">
                        <div class="d-flex align-items-center justify-content-between bg-dark py-2 px-4 mb-3">
                            <h3 class="text-white m-0">Film News</h3>
                        </div>
                    </div>
                    ...
                    <?php else : ?>
                        <?php foreach ($film as $movie) :
                            $text = explode(' ', $movie['body']);
                            $textcut = implode(' ', array_slice($text, 0, 15));
                        ?>
                            <div class="col-lg-4">
                                <div class="position-relative mb-3">
                                    <img class="img-fluid w-100" src="../img/<?= $movie['img']; ?>" style="width: 400px; height: 250px; object-fit: cover;">
                                    <div class="overlay position-relative bg-light">
                                        ...
                                        <a class="h4" href="single.php?id=<?= $movie['id'] ?>"><?= $movie['judul'] ?></a>
